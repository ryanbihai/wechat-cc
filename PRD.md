...
|------|------|----------|-------------------|------|
| **iLink** | 微信机器人会话凭证 | GW 进程内 | `wxuser_abc...@im.wechat` | `wechat/accounts/{id}-im-bot.json` |
| **Bot OB** | Gateway 发/收 OB 消息用的身份 | GW 进程内 | `botOb...` | `bot-ob.json` |
| **wxOB** | 微信用户永久 OB 身份 | GW 进程内 | `wxOb...` | `wx-identity.json` |
| **CC OB** | Claude Code 的 OB 身份 | agent.js 进程 | `ccOb...` | `~/.oceanbus-chat/credentials.json` |
| **CC-alt** | 历史遗留的备选身份 | — | `altOb...` | `.cc-data/credentials.json` |
...
```
Bot OB:  botOb01AbCdEfGhIjKlMnOpQrStUvWxYz0123456789abcdefghijklmnopqrstuvwxyzABCDEFGH
wxOB:    wxOb01AbCdEfGhIjKlMnOpQrStUvWxYz0123456789abcdefghijklmnopqrstuvwxyzABCDEFGH
CC OB:   ccOb01AbCdEfGhIjKlMnOpQrStUvWxYz0123456789abcdefghijklmnopqrstuvwxyzABCDEFGH
CC-alt:  altOb02AbCdEfGhIjKlMnOpQrStUvWxYz0123456789abcdefghijklmnopqrstuvwxyzABCDEFGH
```
...
[1] iLink 长轮询
    ...
    返回: { type:"message", chat_id:"wxuser_abc...", text:"测试15" }
  │
  ▼
[2] Gateway: client.on("message", msg)
    ...
    b) 解析前缀 → 无前缀走默认路由 /cc
    c) 查 routes.json → target = "ccOb..."
    d) 创建临时 OB 实例，用 Bot OB 身份发送:
       ob.send("ccOb...", JSON.stringify({
         action: "command",
         text: "测试15",
         meta: {
           from_wx_user: "wxuser_abc...@im.wechat",
           route_prefix: "/cc",
           agent_name: "CC-abc12345",
           message_id: "wx_1715..."
         }
       }))
...
[4] agent.js: ob.startListening(callback)
    ...
    输出到 stdout:
      {"type":"wechat_msg","chat_id":"wxuser_abc...@im.wechat","sender":"wxuser_abc...@im.wechat","text":"测试15","route":"/cc","agent":"CC","time":"19:08:59"}
  │
  ▼
...
[1] MCP 工具调用 reply(chat_id, text)
    工具名: "reply"
    chat_id: "wxuser_abc...@im.wechat"  ← 来自入站消息的 chat_id 字段
    text: "收到：测试15（来自交互会话）"
  │
  ▼
...
```
agent.js 启动
  → ob.send("wxOb...", { action:"announce", meta:{ agent_name, agent_openid, agent_type } })
...
```
cc-agent.cjs → ob.send("botOb...", reply)
...
- 默认 data-dir 使用 .cc-data/（身份 `altOb...`），与 routes.json 不匹配
...
| Gateway → CC Agent | OceanBus | `ob.send("ccOb...", json)` | `ob.startListening(cb)` | ✅ |
| CC Agent → CC 会话 | stdout JSON | `emit({type:"wechat_msg",...})` | Monitor 捕获 stdout | ✅ |
| Agent → Gateway wxOB | OceanBus | `ob.send("wxOb...", announce)` | `obWx.startListening(cb)` | ✅ |
| 系统命令 | iLink 双向 | `client.sendText()` | `client.on("message")` | ✅ |
| Agent → Gateway Bot OB | OceanBus | `ob.send("botOb...", reply)` | `obListener.startListening(cb)` | ❌ 未用 |
...
| **OB 身份** | `ccOb...` (需 --data-dir) | 默认 `altOb...` (需 --data-dir 覆盖) |
...
```json
{
  "ilinkUserId": "wxuser_abc123def456@im.wechat",
  "wxOpenId": "wxOb01AbCdEfGhIjKlMnOpQrStUvWxYz0123456789abcdefghijklmnopqrstuvwxyzABCDEFGH",
  "defaultRoute": "/cc",
  "boundAt": "<当前时间 ISO>"
}
```
...
**原因：** routes.json 指向 `ccOb...`，但 agent 用了 `.cc-data/` 的身份 `altOb...`。Gateway 发送到 `ccOb...`，agent 在 `altOb...` 上监听，收不到。
...
```
~/.oceanbus-chat/
  └── credentials.json               CC OB 身份 (ccOb...) ★ agent.js --data-dir 必须指向这里

~/.claude/channels/wechat-cc/
  ├── wx-identity.json               微信用户永久 OB 身份 (wxOb...)
  ├── bot-ob.json                    Gateway Bot OB 身份 (botOb...)
...
skills/wechat-cc/
  ...
  └── .cc-data/
      └── credentials.json           备选身份 (altOb...) ★ 不要用
```
